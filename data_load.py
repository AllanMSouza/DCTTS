...
def load_data(mode="train", dataset_path):
    '''Loads data
      Args:
    ...
    if mode=="train":
        if "LJ" in dataset_path:
            # Parse
            fpaths, text_lengths, texts = [], [], []
            transcript = os.path.join(dataset_path, 'metadata.csv')
            lines = codecs.open(transcript, 'r', 'utf-8').readlines()
            for num, line in enumerate(lines):
                fname, _, text = line.strip().split("|")

                fpath = os.path.join(dataset_path, "wavs", fname + ".wav")
                fpaths.append(fpath)
    ...
    with tf.device('/gpu:0'):
        ...
        if hp.prepro:
            def _load_spectrograms(fpath):
                ...
                mel = "mels/{}".format(fname)
                mag = "mags/{}".format(fname)
